...
\begin{frame}
\frametitle{Small streams are underrepresented}
...
\frametitle{Analysing chemical concentrations}
	\begin{itemize}
		...
		\item concentrations \textless~LOQ
		\item 
	\end{itemize}
\end{frame}
...
\begin{frame}
\frametitle{Results: Factors influencing risk (landscape)}
	\includegraphics[width = 1\textwidth]{figs/agri_size_eff.pdf}
\end{frame}

\begin{frame}
\frametitle{Results: Factors influencing risk (sampling)}
...
\begin{frame}
\frametitle{Results: Compounds exceeding risk thresholds}
...
\begin{enumerate}
	...
	\item \alert{Agriculture} important factor
	\item \alert{LOQ} gives additional insights
	\item \alert{Underestimated} risk
	\item Annual \alert{dynamics}
	\item \alert{Neonicotinoids} + Chlorpyrifos
\end{enumerate}
\end{frame}
...
\frametitle{taxize \& webchem: Advances for you (researchers)}
	\begin{enumerate}
		\item saving of time % data cleaning takes 80% of resources
		\item less errors
